...
export default function LoginPage() {
  ...
  const [tokenProcessed, setTokenProcessed] = useState(false); // Thêm flag này
  ...
  useEffect(() => {
    ...
    // Ngăn chặn xử lý lặp lại
    if (!token || tokenProcessed) return;

    setCheckingToken(true);
    setGoogleError('');
    setTokenProcessed(true); // Đánh dấu đã xử lý
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Xóa token khỏi URL trước khi redirect
          window.history.replaceState({}, '', '/');
          router.push('/');
        } else {
          const errorText = await res.text();
          console.error('API Error:', res.status, errorText);
          setGoogleError(`Authentication failed: ${res.status} ${res.statusText}`);
        ...
        setGoogleError('Network error during authentication');
  ...
  }, [login, router, searchParams, tokenProcessed]); // Thêm tokenProcessed vào dependencies
  ...
  const handleGoogleLogin = () => {
    ...
    setGoogleError('');
    setTokenProcessed(false); // Reset flag khi bắt đầu flow mới
    const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:4000';
    window.location.href = `${backendUrl}/api/auth/google`;
  };

  // Hiển thị loading khi đang xử lý Google token
  if (checkingToken) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 p-8">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-slate-700 font-medium">Đang xác thực Google...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4 py-8">
        <div className="w-full max-w-md">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 overflow-hidden">
            {/* Header Section */}
            <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 px-8 py-8 text-center relative">
            ...
            <div className="p-8">
              ...
              {googleError && (
                <div className="bg-red-50 border-2 border-red-200 rounded-xl p-4 mb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-bold text-red-800">Google Login Failed</h4>
              ...
              {isGoogleLoading && (
                <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-blue-800 font-medium">Redirecting to Google...</span>
                  </div>
                </div>
              )}
              ...
              <div className="mb-6">
                <button
                  ...
                  disabled={isGoogleLoading || loading || checkingToken}
              ...
              {/* Form và các phần còn lại giữ nguyên như code gốc */}
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-4 border-2 border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 bg-white text-slate-800 placeholder-slate-400 font-medium hover:border-slate-400"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-12 py-4 border-2 border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 bg-white text-slate-800 placeholder-slate-400 font-medium hover:border-slate-400"
                      required
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  disabled={loading || isGoogleLoading || checkingToken}
                ...
                {error && (
                  <div className="bg-red-50 border-2 border-red-200 rounded-xl p-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="font-bold text-red-800">Login Failed</h4>
                        <p className="text-red-700 text-sm">{error.message || 'Invalid email or password'}</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="text-center mt-8">
                <p className="text-slate-600">
                  ...
                  <Link
                    href="/register"
                    className="text-blue-600 hover:text-blue-800 font-bold transition-colors duration-300"
                  >
                    Sign up now
                  </Link>
                </p>
