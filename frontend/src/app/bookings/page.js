...
function FavoriteButton({ tourId }) {
  ...
  const { data: isFavoriteData, loading: checkingFavorite, refetch: refetchIsFavorite } = useQuery(IS_FAVORITE, {
    variables: { tourId },
    errorPolicy: 'ignore',
    skip: !tourId,
    fetchPolicy: 'cache-and-network', // ✅ Đảm bảo check cache và network
    notifyOnNetworkStatusChange: true // ✅ Notify khi có thay đổi
  });
  ...
  const [toggleFavorite, { loading: toggling }] = useMutation(TOGGLE_FAVORITE, {
    ...
    onCompleted: async (data) => {
      ...
      // ✅ Refetch lại IS_FAVORITE query sau khi toggle
    ...
    // ✅ Update cache sau khi có response
    update: (cache, { data }) => {
      if (data?.toggleFavorite?.success) {
